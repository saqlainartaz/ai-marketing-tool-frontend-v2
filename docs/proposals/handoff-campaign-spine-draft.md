...
export type Play = {
  ...
  /** Present on every play imported from a third-party library. See §7.4 —
   *  this is what generates THIRD-PARTY-NOTICES.md, so it is required, not
   *  decorative. Absent only on plays InsideSuccess authored itself. */
  imported?: {
    repo: string;          // "coreyhaines31/marketingskills"
    path: string;          // "skills/marketing-plan/SKILL.md"
    commit: string;        // pinned SHA — proves what was imported and when
    licence: "MIT" | "Apache-2.0";
    holder: string;        // "Corey Haines, 2025"
...
> **Visual contract (added 2026-08-06).** The approved design direction and the
> exact structure of `/now` live in `handoff-design-direction-draft.md` §2, with a
> built reference at `mockup-now-quiet.html`. Where this section and that mockup
> disagree on layout, the mockup wins — it was reviewed and preferred. This section
> governs the *information architecture*; the mockup governs the *screen*.
...
**Revised 2026-08-06 after a Precedent research run.** The original draft called
for six hand-authored plays. That is the wrong shape for a business serving many
different kinds of client: six plays cover two personas and leave the rest with
generic output. The library must be broad enough that the matcher genuinely
adapts — which means importing, not authoring.

### 7.1 Where plays come from

A large, permissively-licensed ecosystem of marketing Agent Skills already
exists. A search across 16 query angles found 388 repositories, 278
marketing-relevant, of which **121 carry a permissive licence and 141 carry no
licence at all**. The no-licence ones cannot be used — no licence means no grant
of rights, however good the content looks.

Recommended import sources, every licence verified by reading the file rather
than trusting the GitHub badge:

| Repo | Licence | Holder | Stars / contribs | What it contributes |
|---|---|---|---|---|
| coreyhaines31/marketingskills | MIT | Corey Haines, 2025 | 43,134 / 23 | The strategy spine. 36 skills incl. marketing-plan (fCMO-level, AARRR-structured), marketing-ideas (139-idea library), offers, pricing, marketing-psychology, marketing-loops, cro, launch, lead-magnets, churn-prevention, customer-research |
| OpenClaudia/openclaudia-skills | MIT | OpenClaudia, 2026 | 617 / 2 | 73 channel-level plays: content-calendar, content-repurposing, email-sequence, referral-program, google-reviews, reddit-marketing, linkedin-content, podcast-marketing, icp-builder, growth-strategy |
| blacktwist/social-media-skills | MIT | SMS Contributors, 2026 | 396 / 1 | 14 asset-level writers: hook-writer, caption-writer, carousel-writer, thread-writer, content-repurposer, platform-strategy. Maps to the asset layer rather than the play layer |
| aaron-he-zhu/aaron-marketing-skills | Apache-2.0 | (see NOTICE) | 2,515 / 2 | Email and paid-ads operations: subject-line-lab, deliverability-qa, reactivation-specialist, list-hygiene, bid-strategy-planner, budget-pacing-monitor |
| minhnv0807/ai-business-skills | MIT | Over Powers Agency, 2025 | 523 / 2 | A numbered plan pipeline (00-marketing-plan → 03-performance-eval) plus personal-brand and thought-leadership tracks. The numbering is itself a play sequence |
| Varnan-Tech/opendirectory | MIT | Varnan-Tech, 2026 | 573 / 5 | The graphic asset skills: graphic-chart, graphic-case-study, graphic-slide-deck, graphic-ebook. Feeds §8's template pipeline |
| nexscope-ai/eCommerce-Skills | MIT | Nexscope AI, 2026 | 594 / 6 | 151 ecommerce/marketplace plays. Import only if InsideSuccess has ecommerce clients — otherwise it is 151 irrelevant rows |

**Do not import `anthropics/skills`.** It is the format reference and worth
reading, but it has **no licence file** at root — `LICENSE`, `LICENSE.md`,
`LICENSE.txt` and `LICENCE` all 404 as of 2026-08-06. Treat its content as
unlicensed. Also skip `kostja94/marketing-skills` (MIT, 171 skills) despite the
volume: it is a single-contributor website checklist (`404`, `favicon`,
`breadcrumb`) rather than marketing plays, and would pad the catalogue without
improving the match.

### 7.2 The source format, and the import mapping

Skills are standard Agent Skills — YAML frontmatter with `name` and a rich
trigger `description`, then a markdown body giving the role, when-to-use, and
output format. Mapping to the `Play` type in §3.2:

| Skill file | `Play` field |
|---|---|
| `name` | `id` and `name` |
| `description` | seeds `clientSentence` — **must be rewritten** in plain client-facing words per `UX_RULES` §9; the source text is written to trigger an agent, not to reassure a business owner |
| body's when-to-use section | seeds `fitsBusinessModels` / `fitsMotions` — **human-authored, see 7.3** |
| body's output section | seeds the `recipe` and its `promptRef` |
| the whole file | stored as the generation prompt behind `promptRef` |

### 7.2b Revised 2026-08-06 — author the prompts, import only where licensed

After surveying the ecosystem, the balance shifted. Two findings changed it:

1. **Roughly half the ecosystem is unlicensed**, and two of the most-cited
   "unlicensed" marketing libraries (`LeoYeAI/openclaw-marketing-skills`,
   `davidpc007/openclaw-marketing-skills`) are near-identical 39-skill re-uploads
   of `coreyhaines31/marketingskills` with the MIT licence stripped. Take that
   content from the MIT original; ignore the copies.
2. **The ecosystem is built for SaaS, not for these clients.** The recurring
   skills — `paywall-upgrade-cro`, `signup-flow-cro`, `activation-funnel`,
   `programmatic-seo`, `onboarding-cro`, the GA4/GTM audit family — describe a
   business none of the five personas runs. A roofer has no paywall.

So the model is: **survey everything for ideas, author the prompts in-house, copy
prose only from licensed sources under §7.4's attribution rules.** Skill names,
taxonomies and methods are ideas and freely usable as research regardless of
licence; skill prose is protected expression and is not copied unless licensed.

This is also the better build. An imported skill knows nothing about `objection`
atoms, voice profiles or a `claims_blacklist`; a play written in-house uses all
three, and can be made to emit the `Rationale` lines the UI depends on.

**The catalogue lives in `handoff-play-catalogue-draft.md`** — 32 plays across
seven clusters, mapped to the five personas, with a coverage check and the
six-section prompt structure every play shares.

### 7.3 What InsideSuccess actually authors

This is the important point. Importing gives you prompts. It does not give you
the thing that makes the product work, which is **the mapping from a client's
business model and acquisition motion to the plays that suit them.** No imported
skill knows that a roofer winning work by referral needs different plays than a
coach selling a programme; that judgement is yours and it is the whole product.

So the authoring task is not writing 100 prompts. It is tagging them — assigning
`fitsBusinessModels`, `fitsMotions`, `clientEffort` and `requiredAtomTypes` to
each imported play, and rewriting each `clientSentence` in plain words. That is
hours of curation per batch, not weeks of writing, and it is the part that
compounds.

Import in curated batches, not wholesale. A catalogue of 40 well-tagged plays
beats 300 untagged ones, because the matcher's output quality is bounded by tag
quality, not catalogue size. **Wholesale import would recreate the exact
hollowness this spec exists to fix** — a prompt grab-bag with no argument.

### 7.4 Licence obligations — not optional

MIT and Apache-2.0 both require the copyright notice and licence text to travel
with "substantial portions" of the work. A bundled skill prompt is a substantial
portion. Therefore:

- Every imported play carries an `imported` provenance block (§3.2) recording
  repo, path, commit SHA, licence and copyright holder.
- Ship a `THIRD-PARTY-NOTICES.md` generated from those blocks, listing every
  source repo with its licence text and holder.
- Add a row per source repo to the licence ledger in `docs/DECISIONS.md`, which
  already exists for exactly this purpose.
- Apache-2.0 sources additionally need any upstream `NOTICE` file content
  preserved.
- **Never import a no-licence repo**, and re-verify at import time — a repo can
  add or change its licence between research and import.

This is a real obligation, cheap to satisfy now and expensive to retrofit once
prompts are scattered through the codebase.

### 7.5 Two plays worth authoring from scratch

Almost everything can be imported. Two cannot, because they depend on assets
only InsideSuccess has:

- **Episode amplification** — turns the footage you already own into quote
  graphics, a long-form piece and a post series, all grounded in
  transcript-derived atoms. Expressed not as a business-model fit but as
  `requiredAtomTypes: ["quote", "insight"]`: a client with no filmed episode has
  no transcript atoms, so matcher rule 3 excludes it automatically. Eligibility
  falls out of the corpus rather than needing a flag. **No competitor surveyed in
  `PRODUCT_DIRECTION` §5 can run this play, because none of them film their
  clients.** It should be the play in every demo.
- **Objection series** — three posts, each answering one `objection` atom drawn
  from the client's own sales calls. Trivially simple, and impossible for anyone
  without the engine's typed extraction.

Both are short to write and are the two plays that are genuinely yours.
...
Rules 1–4 are hard filters and stay hard: they are what stop a play running
without its inputs. Rule 5 is where a bigger catalogue needs more than counting.

**Ranking at catalogue scale (worth doing, not now).** Once the library passes
roughly 40 plays, overlap-counting will rank several plays identically. The
engine already embeds text with Voyage into pgvector — so the natural upgrade is
to embed each play's `description` and rank by similarity against the client's
Brief and atom corpus, with rules 1–4 still applied as hard filters first. That
reuses infrastructure you already run, and it is what makes the selection feel
genuinely adaptive across many kinds of business rather than table-driven.

Keep it out of the first build. Deterministic ranking is testable as a pure
function; embedding-based ranking is not, and the matcher's correctness matters
more than its sophistication until the catalogue is big enough to need it.
...
**S0 — Import and tag the play catalogue.** A content task, not a code task, and
it can run in parallel with everything else. Pull a curated batch from the §7.1
sources at pinned commits, fill the `imported` block for each, rewrite every
`clientSentence` in plain words, and hand-tag `fitsBusinessModels` / `fitsMotions`
/ `clientEffort` / `requiredAtomTypes`. Generate `THIRD-PARTY-NOTICES.md` and add
the ledger rows. Write Episode amplification and Objection series by hand.
Target ~40 plays. **The matcher cannot be meaningfully tested until this exists**,
so it comes first even though it produces no UI.
...
kinds only). Build the matcher against the S0 catalogue. All pure, all
unit-tested.
...
- **Embedding-based play ranking.** Deferred until the catalogue outgrows
  deterministic ranking — see §7's matcher note. The infrastructure (Voyage +
  pgvector) already exists, so this is a scheduling decision, not a capability gap.
- **Wholesale skill import.** Curated batches only, per §7.3.
...
3. ~~**Who writes the six seed plays?**~~ **Settled 2026-08-06.** Plays are
   imported from permissively-licensed Agent Skill libraries and *tagged* by
   InsideSuccess; only Episode amplification and Objection series are authored
   in-house. See §7. The remaining sub-question is **which batch to import
   first.** *Recommendation:* coreyhaines31/marketingskills for the strategy
   layer plus OpenClaudia/openclaudia-skills for channel plays — around 40 plays
   after curation, which is enough to make the matcher meaningful without
   becoming a grab-bag. Add nexscope-ai/eCommerce-Skills only if ecommerce
   clients actually exist in the base.
...
| Plays imported, not authored; licence obligations | Precedent research run, 2026-08-06 — appendix below |

**Epistemic note.** The diagnosis in §1 comes from reading the
`marketing-tool-v2` code and docs plus a brainstorming session with the product
owner on 2026-08-05. It has not been validated against a real client using the
product, because no client has used it yet. The persona claims it leans on are
sourced in `PRODUCT_DIRECTION` and inherit that document's caveats.

---

## Appendix — the play-library research run (2026-08-06)

Run with Precedent (`workflows/research_similar_repos.md`) against the question
"where do marketing plays come from?".

**Method.** 16 clustered queries: claude skills marketing · marketing agent
skills · social media skills · claude skill social · marketing prompts library ·
prompt library marketing · copywriting prompts · marketing playbook · growth
playbook · awesome claude skills · agent skills collection · lead generation
prompts · local business marketing · email marketing prompts ·
`topic:claude-skills` · `topic:agent-skills`. Every query returned 30 results,
which is the `--limit 30` cap — so **every one was truncated, not exhausted**,
and the real pool is larger than the 388 found.

**Counts.** 388 unique repos · 278 marketing-relevant on a keyword filter · 121
permissively licensed · **141 with no licence at all** · 16 under other terms
(several NOASSERTION, two AGPL-3.0). 13 deep-fetched, 8 licence files read in
full.

**The finding that matters.** Roughly half of the marketing-skill ecosystem is
unlicensed, and the unlicensed half contains some of the most attractive-looking
repositories — `BrianRWagner/ai-marketing-claude-code-skills` ("Marketing
frameworks that AI actually executes"), `stevenflanagan1/social-ai-team` ("a
complete AI social team for SMBs"), `cognyai/claude-code-marketing-skills`. None
of them can be used in a commercial product. Verify a licence before reading a
repo, not after, or the good ideas contaminate the design.

**Caveats.** This proves what exists on GitHub, not what exists anywhere.
Relevance was judged from names, descriptions and topics — GitHub indexes those
far more heavily than README bodies, so a well-licensed library that describes
itself in different vocabulary would have been missed. Skill *quality* was
assessed from one sampled `SKILL.md` per candidate plus contributor and activity
signals; no library was evaluated by running it. Star counts in the agent-skills
ecosystem are unusually inflated and were not treated as quality evidence.
